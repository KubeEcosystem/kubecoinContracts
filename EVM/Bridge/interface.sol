...
interface ILocking20 {
    ...
    /// @dev Emitted when contract admin updates minimum locking amount.
    event UpdatedMinLockAmount(uint256 oldAmount, uint256 newAmount);

    /**
     *  @notice User Info.
     *
     *  @param amountLocked             Total number of tokens locked by the user.
     *
     *  @param timeOfLastUpdate         Last update timestamp.
     */
    struct User {
        uint128 timeOfLastUpdate;
    ...
    /**
     *  @notice View amount locked for a user.
     *
     *  @param user    Address of the check.
     */
    function getLockInfo(address user) external view returns (uint256 _tokensLocked);
}
